...
export class TransferOrderComponent implements OnInit {
  ...
  getTransferOrder() {
    ...
    .getData(`${PATH.TRANSFER}/${this.currentId}`)
      ...
      this.transferForm.get('dueDate').setValidators(null);
      ...
      .subscribe(
        (res: any) => {
          if (res.length) {
            ...
            if(num){
              num.expiryDate=new Date(num.expiryDate);
...
onProductSelection(item, product) {
  ...
  savetransferOrder(form) {
    ...
    payload.transferOrderItems.map((elm)=>{
  ...
  lotSelected(f) {
    ...
    if (num) {
      ...
      if (num.expiryDate) {
        let date=new Date(num.expiryDate);
        f.get('expiryDate').setValue(date);
      } else {
